...
const MessengerContext_1 = __importDefault(require("../context/MessengerContext"));
const MessengerEvent_1 = __importDefault(require("../context/MessengerEvent"));
class MessengerConnector {
    constructor(options) {
        ...
        const { appId, appSecret, mapPageToAccessToken, verifyToken, batchConfig, skipProfile, } = options;
    ...
    _profilePicExpired(user) {
        try {
            const ext = new url_1.URL(user.profile_pic).searchParams.get('ext');
            if (!ext)
                return true;
